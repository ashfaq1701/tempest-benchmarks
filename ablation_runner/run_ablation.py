...
Two-phase design:
  Phase 1 (tune)       — sweep --w-threshold-warp ∈ {1,2,4,8,16,32,64} for
                         **node_grouped only**. node_grouped_global_only
                         shares the same scheduler so it inherits the winning
                         W; tuning it separately would just double-count the
                         same signal. FW ignores the parameter entirely.
                         TUNE_RUNS_PER_W runs per (dataset, W). Picks ONE
                         universal W across datasets by averaging the
                         per-row-normalized **steps/sec**; ties (within
                         NOISE_BAND) break to the smallest W (most
                         conservative).
...
For nsys-derived kernel/NVTX metrics, run `run_profiling.py` AFTER this
script finishes. It reuses the constants/helpers defined here, hardcodes
the winning W, profiles each (dataset, variant), and writes its own CSV.
...
Outputs (CSV; --output BASE produces BASE_tuning.csv and BASE_final.csv):
  ...
  - <base>_final.csv    : one row per (dataset, variant) summarising
                          Phase-2 throughput stats. run_profiling.py reads
                          the winning W from here for its nsys passes.
...
# Phase 1 tunes node_grouped only; node_grouped_global_only inherits the
# winning W in Phase 2 (same scheduler, same W-partition).
...
# Tightened to 0.5% — at 1% the band swallowed almost every W on the
# laptop traces, making "smallest W" the de-facto answer regardless of signal.
NOISE_BAND           = 0.005
...
#                   walk-output VRAM in check on 33.8 M nodes; active graph
#                   at the 5% window stays around ~1 GB.
...
def main() -> int:
    ...
    print(f'# tune variants    : {TUNE_VARIANTS}  (NG_global_only inherits; FW skipped)')
    print(f'# tune runs per W  : {TUNE_RUNS_PER_W}')
    print(f'# tie-break band   : {NOISE_BAND:.1%}  (smallest W wins within this gap)')
    ...
    # ===========================================================
    # Phase 1: W-threshold tuning sweep (node_grouped only).
