#!/usr/bin/env python3
"""
Profiling runner for the second dataset batch (wiki-talk-temporal,
ml_tgbl-comment, sx-stackoverflow). Mirrors run_profiling.py — only the
DATASETS / PRESETS overlay differs (sourced from constants_new.py).

Use AFTER run_ablation_new.py has produced <base>_final.csv.

Usage:
  python3 run_profiling_new.py wiki-talk.csv tgbl-comment.csv \\
      sx-stackoverflow.csv --base ablation_results_new \\
      --block-dim 256 --nsys /its/home/ms2420/cuda-12.6/bin/nsys

  # Re-extract from existing .nsys-rep files without re-profiling:
  python3 run_profiling_new.py ... --reuse-existing
"""
import argparse
import csv
import sqlite3
...
def _extract_ingest(db):
    """Ingest-side metrics from `ingestion_batch` NVTX ranges."""
    nvtx_tbl = _find_nvtx_table(db)
    ranges = db.execute(
...
W_FW_HARDCODED = 1
DEFAULT_BASE   = 'ablation_results_new_2'
...
def invoke_nsys(nsys_bin, rep_path, binary, data, klt, wpn, nb, nw, mwl,
    ...
    """Run binary under `nsys profile --stats=true`. See run_profiling.py
    for the rationale on --stats=true (sidesteps a known bug where
    `nsys export` on a --stats=false rep returns exit 0 but writes a
    sqlite with zero tables)."""
...
def validate_sqlite(rep_path):
    ...
    if not sql_path.exists():
        raise RuntimeError(
            ...
            f'profiled without --stats=true (re-profile to fix).')
...
def main() -> int:
    ...
    ap.add_argument('--reuse-existing', action='store_true',
                    help='Skip nsys profile; assume <nsys_dir>/<ds>_<v>_run<r>'
                         '.nsys-rep already exists. Still re-extracts.')
    args = ap.parse_args()
    ...
    for ds in DATASETS:
        ...
        for variant in ALL_VARIANTS:
            ...
            for r in range(NSYS_RUNS_PER_CELL):
                ...
                tag = f'  {ds:>17} / {variant:<26} / W={w:<3} run {r+1}/{NSYS_RUNS_PER_CELL}'
    ...
    if ingest_rows:
        ...
        print(f'| {"dataset":<17} | {"n":>3} | '
              ...
              f'{"launches":>8} | {"std":>7} |')
        print('|' + '|'.join('-' * w for w in
              [19, 5, 12, 9, 11, 9, 8, 12, 9, 10, 10, 9, 8, 10, 9]) + '|')
        for r in ingest_rows:
            print(f'| {r["dataset"]:<17} | {r["n_samples_kept_total"]:>3} | '
