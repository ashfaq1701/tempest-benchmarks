#!/usr/bin/env python3
"""
Kernel-firing breakdown for the second dataset batch (wiki-talk-temporal,
ml_tgbl-comment, sx-stackoverflow). Mirrors kernel_breakdown.py — only
the DATASETS / PRESETS overlay differs (sourced from constants_new.py).
...
Buckets reported (same as kernel_breakdown.py):
  solo, warp_smem, warp_global, block_smem, block_global, multi_block,
  start_edges, filter_alive, gather, partition_w, partition_g, other.

Usage:
  python3 kernel_breakdown_new.py wiki-talk.csv tgbl-comment.csv \\
      sx-stackoverflow.csv --variant node_grouped --w 4 \\
...
def main() -> int:
    ...
    ap.add_argument('--rep-dir',    default='kernel_breakdown_new_nsys',
